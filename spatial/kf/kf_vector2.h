...
namespace kf
{
	...
	public:
		...
		inline Vector2T(TT xx = 0, TT yy = 0) : x(xx), y(yy)
		{
		}
		...
		friend inline Vector2T normalised(const Vector2T &v)
		{
			Vector2T temp(v);
			temp.normalise();
			return temp;
		...
		inline void set(TT xx = 0, TT yy = 0)
		{
			x = xx;
			y = yy;
		}
		...
		inline Vector2T min(const Vector2T &v) const
		{
			return Vector2T((x<v.x ? x : v.x), (y<v.y ? y : v.y));
		}

		friend inline Vector2T min(const Vector2T &v1, const Vector2T &v2)
		{
			return v1.min(v2);
		}

		inline Vector2T max(const Vector2T &v) const
		{
			return Vector2T((x>v.x ? x : v.x), (y>v.y ? y : v.y));
		}

		friend inline Vector2T max(const Vector2T &v1, const Vector2T &v2)
		{
			return v1.max(v2);
		}
